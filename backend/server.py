...
async def _send_groupme_message_with_rsvp(bot_id: str, text: str, event_id: str, image_url: str = None) -> bool:
    ...
    try:
        ...
        base_url = os.environ.get('REACT_APP_BACKEND_URL', 'https://leaguemate-2.preview.emergentagent.com')
...
@api_router.post("/team/{team_id}/add-player")
async def add_player_to_team(team_id: str, data: Dict[str, Any]):
    ...
    try:
        ...
        if any(ta.get("teamId") == team_id for ta in team_assignments):
        ...
        new_assignment = {
            ...
            "isPrimary": len(team_assignments) == 0  # First team is primary
        }
        team_assignments.append(new_assignment)
        ...
        # Also update legacy fields if this is their first/primary team
        if new_assignment["isPrimary"]:
            update_data["teamId"] = team_id
            update_data["teamName"] = team.get("name")
        
        # Make sure user has player role
        roles = user.get("roles", [])
        if "player" not in roles:
            ...
            update_data["role"] = max(roles, key=lambda r: role_priority.get(r, 0))
        ...
        logger.info(f"✅ Player {user_id} added to team {team_id}")
        
        return {"status": "success", "message": "Player added to team"}
...
@api_router.post("/team/{team_id}/remove-player")
async def remove_player_from_team(team_id: str, data: Dict[str, Any]):
    ...
    try:
        ...
        # Find and remove the team assignment
        original_len = len(team_assignments)
        team_assignments = [ta for ta in team_assignments if ta.get("teamId") != team_id]
        
        if len(team_assignments) == original_len:
            raise HTTPException(status_code=400, detail="Player is not on this team")
        
        # Update primary flag if needed
        if team_assignments and not any(ta.get("isPrimary") for ta in team_assignments):
            team_assignments[0]["isPrimary"] = True
        ...
        if team_assignments:
            primary = next((ta for ta in team_assignments if ta.get("isPrimary")), team_assignments[0])
            update_data["teamId"] = primary.get("teamId")
            update_data["teamName"] = primary.get("teamName")
        else:
            update_data["teamId"] = None
            update_data["teamName"] = None
        ...
        logger.info(f"✅ Player {user_id} removed from team {team_id}")
...
@api_router.post("/team/{team_id}/update-player")
async def update_player_on_team(team_id: str, data: Dict[str, Any]):
    """Update a player's details on a specific team"""
    try:
        user_id = data.get("userId")
        if not user_id:
            raise HTTPException(status_code=400, detail="userId is required")
        ...
        for ta in team_assignments:
            if ta.get("teamId") == team_id:
                if "playerNumber" in data:
                    ta["playerNumber"] = data["playerNumber"]
                if "position" in data:
                    ta["position"] = data["position"]
                if "photoUrl" in data:
                    ta["photoUrl"] = data["photoUrl"]
                found = True
                break
        ...
        # Also update legacy fields if this is their primary team
        primary = next((ta for ta in team_assignments if ta.get("isPrimary")), None)
        if primary and primary.get("teamId") == team_id:
            if "playerNumber" in data:
                update_data["playerNumber"] = data["playerNumber"]
            if "position" in data:
        ...
        logger.info(f"✅ Player {user_id} updated on team {team_id}")
        ...
        logger.error(f"❌ Error updating player on team: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
@api_router.put("/finance/transactions/{transaction_id}")
...
@api_router.get("/finance/team/{team_id}/summary")
...
@api_router.get("/finance/league/summary")
