...
const MediaGallery = ({ teams = [], teamId = null, title = "Media Gallery" }) => {
    // CRITICAL DEBUG - This should show up in console if component is called
    console.log('🚨 MEDIAGALLERY COMPONENT CALLED!', { teams: teams.length, teamId, title });
    ...
    const getGalleries = () => {
        ...
        if (teamId) {
            // For team pages, show:
            // 1. Team-specific galleries (teamId === teamId)
            // 2. League-wide galleries (teamId === 'league-wide')
            return allGalleries.filter(gallery => 
                gallery.teamId === teamId || gallery.teamId === 'league-wide'
            );
        } else {
            // For league homepage, show:
            // 1. League page only galleries (teamId === 'league-only')  
            // 2. League-wide galleries (teamId === 'league-wide')
            return allGalleries.filter(gallery => 
                gallery.teamId === 'league-only' || gallery.teamId === 'league-wide'
            );
        }
    };
    ...
    // Debug logging
    console.log('🖼️ MediaGallery Debug:', {
        teamsCount: teams.length,
        galleriesCount: galleries.length,
        photoGalleriesCount: photoGalleries.length,
        videoGalleriesCount: videoGalleries.length,
        teamId: teamId,
        title: title
    });

    // Always show tabs - don't hide upload functionality when no galleries exist
    // FORCE RENDER TABS FOR DEBUGGING
    return (
        <div className="bg-white rounded-lg shadow-sm border">
            <div className="border-b px-6 py-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex space-x-2">
                        ...
                        >
                            <svg className="mr-1 inline w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-6">
                ...
                {activeTab === 'youtube' && (
                    <YouTubeGallery 
                        teamId={teamId} 
                        title={teamId ? "Team YouTube Videos" : "League YouTube Videos"} 
                    />
                )}
                ...
                {activeTab === 'upload' && (
                    <GoogleDriveUploader 
                        ...
                        onUploadSuccess={(uploadedFiles) => {
                            ...
                            // TODO: Refresh galleries to show new files
                        }}
                    />
                )}
            </div>
            ...
            <div className="mb-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        {showTeamName && gallery.teamName && gallery.teamId !== 'league-only' && gallery.teamId !== 'league-wide' && (
