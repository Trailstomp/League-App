...
const WebsiteDesignManager = React.memo(({ websiteStyle = {}, setWebsiteStyle, teams = [], events = [], tickerConfig = {}, onTickerConfigChange }) => {
    ...
    const handleSave = useCallback(async (overrideFields) => {
        try {
            ...
            // Merge any override fields on top of the current ref state
            const currentState = overrideFields ? { ...baseState, ...overrideFields } : baseState;
            ...
            <div className="border-t pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <ColorPickerWithTexture
                        ...
                        colorValue={editingStyle.menuBackgroundColor}
                        textureValue={editingStyle.menuBackgroundColorTexture}
                        onColorChange={(val) => updateStyle({ menuBackgroundColor: val, menuBackgroundColorTexture: '' })}
                        onTextureSelect={(id) => updateStyle({ menuBackgroundColorTexture: id || '' })}
                        fieldName="menuBg"
                    />
                    <ColorPickerWithTexture
                        label="Menu Text Color"
                        colorValue={editingStyle.menuTextColor}
                        textureValue={editingStyle.menuTextColorTexture}
                        onColorChange={(val) => updateStyle({ menuTextColor: val, menuTextColorTexture: '' })}
                        onTextureSelect={(id) => updateStyle({ menuTextColorTexture: id || '' })}
                        fieldName="menuText"
                    />
                </div>
        ...
        <div className="space-y-4">
            ...
            <div className="border rounded-lg overflow-hidden">
                ...
                <div className="mt-4">
                    <ColorPickerWithTexture
                        ...
                        colorValue={editingStyle.inputTextColor || '#1e293b'}
                        textureValue={editingStyle.inputTextColorTexture}
                        onColorChange={(val) => updateStyle({ inputTextColor: val, inputTextColorTexture: '' })}
                        onTextureSelect={(id) => updateStyle({ inputTextColorTexture: id || '' })}
                        fieldName="inputText"
                    />
                    <p className="text-xs text-slate-500 mt-1">Controls the text color inside input fields, dropdowns, and text areas</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                ...
                <div className="mt-4">
                    <ColorPickerWithTexture
                        label="Secondary / Subtitle Text Color"
                        colorValue={editingStyle.secondaryTextColor || '#64748b'}
                        textureValue={editingStyle.secondaryTextColorTexture}
                        onColorChange={(val) => updateStyle({ secondaryTextColor: val, secondaryTextColorTexture: '' })}
                        onTextureSelect={(id) => updateStyle({ secondaryTextColorTexture: id || '' })}
                        fieldName="secondaryText"
                    />
                    <p className="text-xs text-slate-500 mt-1">Controls grey labels, descriptions, dates, and other subtitle text</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                ...
                <div className="mt-4">
                    <ColorPickerWithTexture
                        ...
                        colorValue={editingStyle.accentTextColor || '#4f46e5'}
                        textureValue={editingStyle.accentTextColorTexture}
                        onColorChange={(val) => updateStyle({ accentTextColor: val, accentTextColorTexture: '' })}
                        onTextureSelect={(id) => updateStyle({ accentTextColorTexture: id || '' })}
                        fieldName="accentText"
                    />
                    <p className="text-xs text-slate-500 mt-1">Controls colored labels, info text, and section accent text</p>
                    <div className="flex flex-wrap gap-2 mt-2">
            ...
            <div className="border rounded-lg overflow-hidden">
                <div className="bg-amber-50 px-3 py-2 border-b flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-semibold text-amber-800">Interactive Colors</h4>
                    <span className="text-[10px] text-amber-500 ml-auto">Buttons, hover, selected states</span>
                </div>
                <div className="p-3 space-y-3">
                    <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Primary / Button Color</label>
                        <div className="flex items-center gap-2">
                            <input type="color" value={editingStyle.primaryColor || '#3b82f6'} onChange={(e) => updateStyle({ primaryColor: e.target.value })} className="w-8 h-8 rounded border cursor-pointer" />
                            <input type="text" value={editingStyle.primaryColor || '#3b82f6'} onChange={(e) => updateStyle({ primaryColor: e.target.value })} className="flex-1 px-2 py-1 border rounded text-xs font-mono" />
                            <span className="text-[10px] text-slate-400">CTA buttons, links</span>
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Accent Color</label>
                        <div className="flex items-center gap-2">
                            <input type="color" value={editingStyle.accentColor || '#6366f1'} onChange={(e) => updateStyle({ accentColor: e.target.value })} className="w-8 h-8 rounded border cursor-pointer" />
                            <input type="text" value={editingStyle.accentColor || '#6366f1'} onChange={(e) => updateStyle({ accentColor: e.target.value })} className="flex-1 px-2 py-1 border rounded text-xs font-mono" />
                            <span className="text-[10px] text-slate-400">Badges, highlights</span>
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Hover / Highlight Color</label>
                        <div className="flex items-center gap-2">
                            <input type="color" value={editingStyle.hoverColor || editingStyle.primaryColor || '#3b82f6'} onChange={(e) => updateStyle({ hoverColor: e.target.value })} className="w-8 h-8 rounded border cursor-pointer" />
                            <input type="text" value={editingStyle.hoverColor || editingStyle.primaryColor || '#3b82f6'} onChange={(e) => updateStyle({ hoverColor: e.target.value })} className="flex-1 px-2 py-1 border rounded text-xs font-mono" />
                            <span className="text-[10px] text-slate-400">Nav hover, row highlight</span>
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-slate-600 mb-1">Active / Selected Color</label>
                        <div className="flex items-center gap-2">
                            <input type="color" value={editingStyle.activeColor || editingStyle.primaryColor || '#2563eb'} onChange={(e) => updateStyle({ activeColor: e.target.value })} className="w-8 h-8 rounded border cursor-pointer" />
                            <input type="text" value={editingStyle.activeColor || editingStyle.primaryColor || '#2563eb'} onChange={(e) => updateStyle({ activeColor: e.target.value })} className="flex-1 px-2 py-1 border rounded text-xs font-mono" />
                            <span className="text-[10px] text-slate-400">Active nav, selected tab</span>
                        </div>
                    </div>
                    <div className="bg-slate-50 rounded p-2">
                        <div className="text-[10px] text-slate-500 mb-1">Preview</div>
                        <div className="flex gap-2 items-center flex-wrap">
                            <button className="px-3 py-1 rounded text-xs text-white font-medium" style={{ backgroundColor: editingStyle.primaryColor || '#3b82f6' }}>Button</button>
                            <button className="px-3 py-1 rounded text-xs text-white font-medium opacity-80" style={{ backgroundColor: editingStyle.hoverColor || editingStyle.primaryColor || '#3b82f6' }}>Hovered</button>
                            <span className="px-2 py-0.5 rounded text-xs font-medium border-b-2" style={{ color: editingStyle.activeColor || editingStyle.primaryColor || '#2563eb', borderColor: editingStyle.activeColor || editingStyle.primaryColor || '#2563eb' }}>Active Tab</span>
                            <span className="px-1.5 py-0.5 rounded text-[10px] font-medium text-white" style={{ backgroundColor: editingStyle.accentColor || '#6366f1' }}>Badge</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const renderTickerSection = () => (
        <div className="space-y-6">
            <TickerManager 
                ...
                onTickerConfigChange={onTickerConfigChange}
            />
        </div>
    );
