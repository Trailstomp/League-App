    ...
    const testConnection = async () => {
        ...
        try {
            ...
            if (data.error) {
                ...
                setTestResult({ success: true, message: `Found channel: ${data.title}` });
            } else {
                setTestResult({ success: false, message: 'Channel not found or API key not configured' });
            }
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h5 className="font-medium text-amber-800">YouTube API Key Required</h5>
                        <p className="text-sm text-amber-700 mt-1">
                            To fetch videos from YouTube, a YouTube Data API v3 key must be configured in the server environment.
