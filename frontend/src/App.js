...
const TeamSocialMediaManager = ({ team, setTeams }) => {
    ...
    // Team-specific social media credentials
    const [credentials, setCredentials] = useState(() => {
        const stored = localStorage.getItem(`mlbl_team_social_${team?.id}`);
    ...
    const handlePost = async (e) => {
        ...
        // Simulate posting to connected platforms
        const connectedPlatforms = selectedPlatforms.filter(platform => credentials[platform]?.connected);
        
        if (connectedPlatforms.length === 0) {
            alert('Please connect at least one social media platform first.');
            setIsPosting(false);
            return;
        }
        ...
        try {
            ...
            alert(`Posted to ${connectedPlatforms.join(', ')} successfully!`);
            setPostContent('');
        } catch (error) {
    ...
    const togglePlatformConnection = (platform) => {
        const newCredentials = {
            ...
            [platform]: {
                ...
                connected: !credentials[platform].connected
            }
        };
        setCredentials(newCredentials);
        localStorage.setItem(`mlbl_team_social_${team?.id}`, JSON.stringify(newCredentials));
    };
        ...
        <div className="space-y-4">
            ...
            <div className="flex space-x-4 border-b">
                <button 
                    ...
                    className={`pb-2 px-1 ${activeTab === 'overview' ? 'border-b-2 border-blue-600 text-blue-600 font-semibold' : 'text-slate-600'}`}
                >
                    Overview
                </button>
                <button 
                    onClick={() => setActiveTab('post')}
                    className={`pb-2 px-1 ${activeTab === 'post' ? 'border-b-2 border-blue-600 text-blue-600 font-semibold' : 'text-slate-600'}`}
                >
                    Quick Post
                </button>
                <button 
                    onClick={() => setActiveTab('settings')}
                    className={`pb-2 px-1 ${activeTab === 'settings' ? 'border-b-2 border-blue-600 text-blue-600 font-semibold' : 'text-slate-600'}`}
                >
                    Settings
                </button>
            </div>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-4">
                        {['twitter', 'facebook', 'instagram'].map(platform => (
                            <div key={platform} className="bg-white border rounded-lg p-3 text-center">
                                <div className="flex items-center justify-center mb-2">
                                    {platform === 'twitter' && <Twitter className="text-blue-400" size={20} />}
                                ...
                                <p className={`text-xs ${credentials[platform]?.connected ? 'text-green-600' : 'text-slate-400'}`}>
                                    {credentials[platform]?.connected ? 'Connected' : 'Not Connected'}
                                </p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="bg-slate-50 border rounded-lg p-4">
                        <h4 className="font-semibold text-slate-800 mb-3">Current Social Media Links</h4>
                        <div className="space-y-2 text-sm">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-slate-600">{team.social?.twitter || 'Not set'}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="flex items-center space-x-2">
                                ...
                                <span className="text-slate-600">{team.social?.facebook || 'Not set'}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="flex items-center space-x-2">
                                ...
                                <span className="text-slate-600">{team.social?.instagram || 'Not set'}</span>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 mt-2">
                            Update these links in the "Social Media Links" section above to change your public profile links.
                        </p>
                    </div>
                </div>
            )}
                                ...
                                {['twitter', 'facebook', 'instagram'].map(platform => (
                                    <label key={platform} className="flex items-center space-x-2">
                                        <input
                                            ...
                                            disabled={!credentials[platform]?.connected}
                                        />
                                        <span className={`text-sm capitalize ${!credentials[platform]?.connected ? 'text-slate-400' : ''}`}>
                                            {platform}
                                        </span>
                                    </label>
                                ))}
            ...
            {activeTab === 'settings' && (
                <div className="space-y-4">
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <h4 className="font-semibold text-yellow-800 mb-2">Platform Connections</h4>
                        <p className="text-sm text-yellow-700 mb-3">
                            Toggle platform connections for quick testing. In production, this would connect to real APIs.
                        </p>
                        ...
                        <div className="space-y-3">
                            {['twitter', 'facebook', 'instagram'].map(platform => (
                                <div key={platform} className="flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        {platform === 'twitter' && <Twitter className="text-blue-400" size={18} />}
                                        {platform === 'facebook' && <Facebook className="text-blue-600" size={18} />}
                                        {platform === 'instagram' && <Instagram className="text-pink-500" size={18} />}
                                    ...
                                    >
                                        {credentials[platform]?.connected ? 'Connected' : 'Connect'}
                                    </button>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <h4 className="font-semibold text-blue-800 mb-2">Need Full API Setup?</h4>
                        <p className="text-sm text-blue-700 mb-3">
                            For advanced features like scheduling, analytics, and full cross-platform management, 
                            ask your league admin to set up full API credentials in the Admin → Social Media section.
                        </p>
                        <p className="text-sm text-blue-700">
                            This includes Twitter API keys, Facebook page tokens, Instagram business accounts, and YouTube integration.
                        </p>
                    </div>
                </div>
            )}
